...
- **Главная** (`/`) - Dashboard с общей статистикой продаж
- **Продажи** (`/sales`) - Детализация продаж с фильтрацией по датам, магазинам и сортировкой
- **Остатки** (`/stock`) - Текущие остатки товаров на складе с фильтрацией по группам, поиском и пагинацией

## Data Loading Mode

- Интерфейс загружается без обращений к Proxy API.
- Нажмите «Загрузить данные» на главной странице, в разделе продаж или остатков, чтобы инициировать запросы.
- После первой загрузки можно менять фильтры; параметр `load=1` останется в запросе.
- В переменной `PROXY_TIMEOUT` по умолчанию установлено значение `5`, чтобы приложение не зависало при недоступном API; при необходимости увеличьте его.
